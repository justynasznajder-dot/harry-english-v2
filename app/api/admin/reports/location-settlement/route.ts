...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = await fetchLocationSettlement(year, periodMonth || undefined);
    ...
    return NextResponse.json({
      ...
      rows,
      totals_by_location: summarizeLocationRows(rows),
      note: "Liczone są wyłącznie zajęcia ze statusem COMPLETED.",
    });
  } catch (error) {
    console.error("GET reports/location-settlement error:", error);
    return NextResponse.json({ message: "Błąd raportu rozliczenia lokalizacji" }, { status: 500 });
  }
}
