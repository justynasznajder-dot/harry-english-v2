...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createCorrectiveInvoice({
    ...
    console.error("POST /api/accountant/invoices/corrective:", error);
    return NextResponse.json(
      { message: "Nie udało się wystawić faktury korygującej" },
      { status: 500 }
    );
  }
}
