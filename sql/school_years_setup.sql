...
-- Indeks częściowy — tylko jeden aktywny rok na szkołę:
...
ALTER TABLE groups ADD COLUMN IF NOT EXISTS school_year_id UUID REFERENCES school_years (id) ON DELETE SET NULL;
ALTER TABLE lessons ADD COLUMN IF NOT EXISTS school_year_id UUID REFERENCES school_years (id) ON DELETE SET NULL;
ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS school_year_id UUID REFERENCES school_years (id) ON DELETE SET NULL;
