...
interface SignedContractDownload {
  contractId: string;
  childId: string;
...
async function downloadSignedContractPdf(params: {
...
function ChildSignedPdfButtons({ doc }: { doc: SignedContractDownload }) {
  ...
  const btnClass =
    'rounded-full bg-[#0f6e56] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#0b5a46] disabled:cursor-not-allowed disabled:opacity-60';
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2 border-t border-zinc-200 pt-3">
      ...
      >
        {busyKey === 'contract' ? 'PDF…' : 'Pobierz umowę'}
      </button>
      {doc.hasAttachment1 ? (
        <button
          type="button"
      ...
      {doc.hasAttachment2 ? (
        ...
        >
          {busyKey === 'att2' ? 'PDF…' : 'Pobierz zgodę na odbiór'}
        </button>
      ) : null}
    </div>
  );
...
/** Kwoty z propozycji managera (ratalna / jednorazowa) + efektywny % rabatu; 2× w tygodniu = ×2. */
function resolveProposalLessonsPerWeek(p: EnrollmentProposal): LessonsPerWeek {
  const fromField = normalizeLessonsPerWeek(p.lessons_per_week);
  if (fromField) return fromField;
  const slots = String(p.schedule ?? '')
    .split(',')
    .map((s) => s.trim())
    .filter((s) => s.length > 0 && s.toLowerCase() !== 'do ustalenia');
  return slots.length >= 2 ? 2 : 1;
}
...
function resolveComplimentaryManagerAmounts(
  ...
  const lessonsPerWeek = resolveProposalLessonsPerWeek(p);
  return (
    [
    ...
    const unitAmount = resolveProposalPaymentAmount(p, type);
    const baseAmount = scaleAmountByLessonsPerWeek(unitAmount, lessonsPerWeek);
...
export default function EnrollmentParentFlow({
  ...
  const [signedContractDownloads, setSignedContractDownloads] = useState<SignedContractDownload[]>(
    [],
  );
  ...
  >({});
  /** Częstotliwość zajęć (1× / 2×) — ustawiana przez managera; tylko do wyceny w UI. */
  ...
  const loadProposals = useCallback(async () => {
    ...
    try {
      ...
      setSignedContractDownloads(
        Array.isArray(data.signedContractDownloads) ? data.signedContractDownloads : [],
      );
      ...
      if (data.contractPricing) {
        ...
        if (kdr) setSummaryDiscountsLocked(true);
      }
      if (typeof data.enrollingMultipleChildren === 'boolean') {
        setEnrollingMultipleChildren(data.enrollingMultipleChildren);
        setSavedEnrollingMultipleChildren(data.enrollingMultipleChildren);
        if (data.enrollingMultipleChildren) setSummaryDiscountsLocked(true);
      } else {
        const activeChildrenCount = incoming.filter(
          (p) => childAccessLevel(p) !== 'REJECTED',
        ).length;
        const inferred = activeChildrenCount >= 2;
        setEnrollingMultipleChildren(inferred);
        setSavedEnrollingMultipleChildren(inferred);
      }
  ...
  const handleSaveAndSubmitContractData = useCallback(async () => {
    ...
    const needsProfileSave = isEditingContractProfile || !profileComplete;
    if (needsProfileSave) {
    ...
    try {
      ...
      setIsEditingContractProfile(false);
      setSavedDiscountLargeFamily(discountLargeFamily);
                ...
                proposals.some(
                  ...
                  <p className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
                    ...
                    {ENROLLMENT_REQUIRE_PROPOSAL_ACCEPTANCE
                      ...
                      : ' Oczekujemy na podpisanie umowy przez rodzica.'}
                  </p>
                )}
              {!userInfo.complimentaryAccess &&
                  ...
                  >
                    ...
                    {userInfo.complimentaryAccess &&
                      (level === 'PROPOSED' ||
                        ...
                        level === 'NEGOTIATING' ||
                        isActionable) && (
                      <div className="mt-4 flex flex-col gap-2 rounded-xl border border-emerald-100 bg-emerald-50/50 px-3 py-3">
                        <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
                          Rabaty
                        </p>
                        <p className="text-xs text-zinc-500">
                          Kwoty poniżej to stawki wpisane przez szkołę (ratalna i jednorazowa).
                        </p>
                        <label
                          className={`inline-flex items-start gap-2 text-sm ${
                            enrollingMultipleChildren ? 'text-zinc-400' : 'text-zinc-700'
                          }`}
                        >
                          <input
                            type="checkbox"
                            className="mt-0.5 accent-[#0f6e56]"
                            disabled={buttonsDisabled || enrollingMultipleChildren}
                            checked={discountLargeFamily && !enrollingMultipleChildren}
                            onChange={(e) => {
                              ...
                              void (async () => {
                                try {
                                  ...
                                  /* ignore — zapis przy akceptacji/umowie */
                                }
                              })();
                            }}
                          />
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 space-y-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-zinc-600">
                        Dla każdego dziecka wybierz sposób płatności. Umowy generują się po kolei
                        (jedno dziecko = jedna umowa).
                      </p>
                    </div>
                    {(() => {
                      ...
                      return (
                        <>
                          {hasPipelineChild && !billingExempt ? (
                            <div className="space-y-2 rounded-xl border border-white bg-white px-4 py-3">
                              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
                                Zniżki
                              </p>
                              <p className="text-xs text-zinc-500">
                              ...
                              <div className="flex flex-col gap-2">
                                ...
                                >
                                  <input
                                    type="checkbox"
                                    className="mt-0.5 accent-[#0f6e56]"
                                    disabled={settlementRadiosLocked || enrollingMultipleChildren}
                                    checked={discountLargeFamily && !enrollingMultipleChildren}
                                    onChange={(e) => {
                                      const checked = e.target.checked;
                                      setDiscountLargeFamily(checked);
                          ...
                          <div className="space-y-3">
                            {proposals.map((p) => {
                              ...
                              const childLessonsPerWeek =
                                lessonsPerWeekByRequestId[p.request_id] ??
                                resolveProposalLessonsPerWeek(p);
                          ...
                          >
                            <div className="min-w-0 space-y-3">
                              <div>
                                <p
                                  className={`font-semibold ${
                                    isSignedDone ? 'text-zinc-700' : 'text-zinc-900'
                                  }`}
                                >
                                  {p.child_first_name} {p.child_last_name}
                                </p>
                                {p.group_name ? (
                                  <p
                                    className={`mt-1 text-sm ${
                                      isSignedDone ? 'text-zinc-500' : 'text-zinc-600'
                                    }`}
                                  >
                                    {p.group_name}
                              ...
                              {isSignedDone
                                ? (() => {
                                    const signedDoc =
                                      signedContractDownloads.find(
                                        (d) =>
                                          (p.child_id && d.childId === p.child_id) ||
                                          (d.requestId != null && d.requestId === p.request_id),
                                      ) ?? null;
                                    return signedDoc ? (
                                      <ChildSignedPdfButtons doc={signedDoc} />
                                    ) : null;
                                  })()
                                : null}
                                            ...
                                            <span>
                                              ...
                                              {showDiscountNote ? (
                                                <span className="mt-0.5 block text-xs font-normal text-zinc-500">
                                                  Cena wynika z {formatPlnAmount(baseAmount)}
                    ...
                    {contractPreview && !isContractSigned ? (
                      <div className="space-y-4 border-t border-emerald-200 pt-4">
                        <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
                          Zaakceptuj dokumenty w kolejności: wizerunek (opcjonalnie), odbiór (jeśli
                          dotyczy), na końcu umowa.
                        </div>
                        <ContractPortal
                          ...
                          onSigned={async (result) => {
                            ...
                            if (next && pdfOk) {
                              const nextPaymentType =
                                (next.request_id && paymentTypeByRequestId[next.request_id]) ||
                                'MONTHLY';
                              setAllowContractRegenerate(true);
                    ...
                    ) : isContractSigned && signedContractDownloads.length > 0 ? (
                      <p className="border-t border-emerald-200 pt-4 text-sm text-zinc-600">
                        Podpisane PDF-y pobierzesz przy boxie każdego dziecka powyżej.
                      </p>
                    ) : null}
                  </div>
                  )}
          ...
          <div className="space-y-3">
            {userInfo.complimentaryAccess ? (
              (() => {
                ...
                const effectiveSibling =
                  enrollingMultipleChildren && !discountLargeFamily;
                const summaryDiscountsDirty =
                ...
                const saveSummaryDiscounts = async () => {
                  ...
                  try {
                    const res = await fetch('/api/enrollment/discounts', {
                      ...
                      body: JSON.stringify({
                        discountLargeFamily,
                        enrollingMultipleChildren: effectiveSibling,
                      }),
                    });
                    ...
                    if (!res.ok) {
                      setFlash({
                        kind: 'error',
                        message: data.message ?? 'Nie udało się zapisać zniżek',
                      });
                      return;
                    }
                    setSavedDiscountLargeFamily(discountLargeFamily);
                    setSavedEnrollingMultipleChildren(effectiveSibling);
                    setSummaryDiscountsLocked(true);
                    setContractPricing((prev) =>
                      prev ? { ...prev, discountLargeFamily } : prev,
                    );
                    setFlash({ kind: 'success', message: 'Zapisano zniżki' });
                    await loadProposals();
                  } catch {
                    setFlash({
                      kind: 'error',
                      message: 'Nie udało się zapisać zniżek',
                    });
                  } finally {
                    setSavingSummaryDiscounts(false);
                  }
                };
                const discountsDisabled =
                  isReadOnlyPreview || savingSummaryDiscounts || summaryDiscountsLocked;
                return (
                  <>
                    <div className="space-y-2 rounded-2xl border border-emerald-200 bg-white px-4 py-3">
                      <p className="text-sm text-zinc-700">
                        Oznacz odpowiednie pole poniżej jeżeli przysługuje Ci zniżka.
                      </p>
                      <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
                        Zniżki
                      </p>
                      <div className="flex flex-col gap-2">
                        <label
                          className={`inline-flex items-start gap-2 text-sm ${
                            discountsDisabled || enrollingMultipleChildren
                              ? 'text-zinc-400'
                              : 'text-zinc-700'
                          }`}
                        >
                          <input
                            type="checkbox"
                            className="mt-0.5 accent-[#0f6e56]"
                            disabled={discountsDisabled || enrollingMultipleChildren}
                            checked={discountLargeFamily && !enrollingMultipleChildren}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setDiscountLargeFamily(checked);
                        ...
                        <label
                          className={`inline-flex items-start gap-2 text-sm ${
                            discountLargeFamily || discountsDisabled
                              ? 'text-zinc-400'
                              : 'text-zinc-700'
                          }`}
                        >
                          <input
                            type="checkbox"
                            className="mt-0.5 accent-[#0f6e56]"
                            disabled={discountsDisabled || discountLargeFamily}
                      ...
                      {!isReadOnlyPreview && !summaryDiscountsLocked ? (
                        <button
                          type="button"
                          disabled={!summaryDiscountsDirty || savingSummaryDiscounts}
                          onClick={() => {
                            void saveSummaryDiscounts();
                          }}
                          className="mt-1 rounded-full bg-[#0f6e56] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#0b5a46] disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          {savingSummaryDiscounts ? 'Zapisywanie…' : 'Zapisz'}
                        </button>
                      ) : null}
                      {summaryDiscountsLocked && !isReadOnlyPreview ? (
                        <p className="text-xs text-zinc-500">Zniżki zostały zapisane.</p>
                      ) : null}
                    </div>
                    <p className="text-sm text-zinc-600">
