...
export default function MessagesPanel({ mode, currentUserId: currentUserIdProp }: MessagesPanelProps) {
  ...
  const buildRecipientsQuery = useCallback(() => {
    ...
    if (mode === 'manager') {
      ...
      if (!hasBulk) q.set('all', 'true');
        ...
        if (filterGroupId) q.set('groupId', filterGroupId);
        if (filterLocationId) q.set('locationId', filterLocationId);
        if (filterSchoolYearId) q.set('schoolYearId', filterSchoolYearId);
        if (filterTeacherId) q.set('teacherId', filterTeacherId);
        if (filterEnrollmentStatus) q.set('enrollmentStatus', filterEnrollmentStatus);
      }
    }
    return q;
  }, [
    mode,
    recipientSearchDebounced,
    filterGroupId,
    filterLocationId,
    filterSchoolYearId,
    filterTeacherId,
  ...
  const loadRecipients = useCallback(async () => {
    try {
      const res = await fetch(`/api/messages/recipients?${buildRecipientsQuery()}`, {
        cache: 'no-store',
      });
      const data = await res.json();
      ...
      const list = includeTeachers ? [...parents, ...teachers] : parents;
      setRecipients(list);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Błąd odbiorców');
    }
  }, [buildRecipientsQuery, includeTeachers]);

  useEffect(() => {
    if (composeOpen) void loadRecipients();
  }, [composeOpen, loadRecipients, recipientsReloadToken]);
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-emerald-100 bg-white p-5 md:p-6">
            ...
            return (
              <li key={thread.id}>
                <button
                  ...
                  className="flex w-full gap-3 px-4 py-3 text-left transition hover:bg-emerald-50/50"
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#0f6e56] text-sm font-bold text-white">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex flex-wrap items-center gap-2">
                      ...
                      {thread.replyCount > 0 && (
                        <span className="rounded-full bg-[#ffc94a] px-2 py-0.5 text-xs font-bold text-[#3b2a10]">
                          {thread.replyCount}
                        </span>
                      )}
                    ...
                    <span className="mt-0.5 block truncate text-sm font-medium text-zinc-800">
      ...
      <ComposeMessageModal
        ...
        onFilterSchoolYearIdChange={setFilterSchoolYearId}
        filterTeacherId={filterTeacherId}
        onFilterTeacherIdChange={setFilterTeacherId}
        filterEnrollmentStatus={filterEnrollmentStatus}
        onFilterEnrollmentStatusChange={setFilterEnrollmentStatus}
        includeTeachers={includeTeachers}
        onIncludeTeachersChange={setIncludeTeachers}
        onApplyManagerFilters={applyManagerFilters}
        onAddAllFromList={() => recipients.forEach((r) => addRecipient(r))}
