...
import ManagerTopNav, { type ManagerTabKey } from '@/src/components/ManagerTopNav';

export interface PortalShellUserInfo {
  id: string;
  email: string;
  phone?: string | null;
  firstName: string;
...
function PortalHeaderUserBlock({ userInfo }: { userInfo: PortalShellUserInfo | null }) {
  if (!userInfo) {
...
export type PortalAppShellRenderProps = {
  userInfo: PortalShellUserInfo | null;
  onUserInfoUpdate: (updated: PortalShellUserInfo) => void;
};

type PortalAppShellProps = {
  children: ReactNode | ((ctx: PortalAppShellRenderProps) => ReactNode);
  /** Gdy true, pokazuje górne menu zakładek zarządcy (Pulpit, Organizacja…). */
  showManagerNav?: boolean;
  /** Podświetlona zakładka w menu zarządcy (np. na profilu rodzica/dziecka). */
  managerActiveTab?: ManagerTabKey | null;
...
export default function PortalAppShell({
  ...
  managerActiveTab = null,
  maxWidthClassName = 'max-w-6xl',
}: PortalAppShellProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [isHomeBtnHovered, setIsHomeBtnHovered] = useState(false);
  const [userInfo, setUserInfo] = useState<PortalShellUserInfo | null>(null);
  ...
  const handleUserInfoUpdate = useCallback((updated: PortalShellUserInfo) => {
    setUserInfo((prev) => {
      if (!prev) {
        const next = {
          ...updated,
      ...
      const next: PortalShellUserInfo = {
        ...prev,
        ...updated,
  ...
  const managerNavVisible =
    showManagerNav && (userInfo?.role === 'MANAGER' || userInfo?.role === 'ADMIN');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f3c33] to-[#175244] p-4 sm:p-6">
      <div className={`mx-auto ${maxWidthClassName}`}>
        ...
        {managerNavVisible ? (
          <div className="mb-4">
            <ManagerTopNav activeTab={managerActiveTab} />
          </div>
        ) : null}

        {typeof children === 'function'
          ? children({ userInfo, onUserInfoUpdate: handleUserInfoUpdate })
          : children}
      </div>
    </div>
  );
